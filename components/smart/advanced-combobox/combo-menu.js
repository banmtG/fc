...
class ComboMenu extends HTMLElement {
  ...
  connectedCallback() {
    ...
    // Register with FocusStack when menu is focused/opened
    FocusStack.push(this);

    this.bindKeyEvents(); // attach keyboard handling
  ...
  /**
   * Attach keyboard handling for this menu.
   */
  bindKeyEvents() {
  ...
  /**
   * Detach keyboard handling for this menu.
   */
  unbindKeyEvents() {
   ...
   _renderMenu() {
    ...
    this._visibleItems.forEach((vi, idx) => {
      ...
      if (this.hasAttribute('allow-delete')) {
        ...
        this._rowCleanups.add(() => deleteIcon.removeEventListener('click', onDeleteClick));
  ...
  _onKeyDown = (e) => {
    ...
    // 🔹 Esc key: emit event so parent can close popup
    if (e.key === 'Escape') {
      this.dispatchEvent(new CustomEvent('combo-menu-close', {
        bubbles: false,
        composed: true
      }));
      return;
