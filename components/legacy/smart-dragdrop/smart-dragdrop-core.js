...
component._onPointerDown = e => {
  ...
  // NEW: remember which container we started in
...
component._onPointerMove = e => {
  ...
  // Build lasso box in page coordinates
  const box = { left: x, top: y, right: x + w, bottom: y + h };

  // NEW: only select items inside the starting container
...
component._onPointerUp = () => {
  ...
  component._lassoContainer = null; // reset
  component.shadowRoot.getElementById("lasso").style.display = "none";
// Query inside shadowRoot, not the host element 
...
export function syncContainers(component) {
  ...
  component.dispatchEvent(new CustomEvent("dragdrop-box-changed", {
    detail: { columnData: component.columnData }
  }));
}
