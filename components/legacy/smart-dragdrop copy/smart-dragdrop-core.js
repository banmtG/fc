...
component._handleClickSelection = function (e) {
  console.log(`_handleClickSelection dragging`, component._isDragging);
  if (component._justLassoed) { // 👇 skip this click, reset flag 
  ...
  // Case 1: click on empty space → clear all
  if (!item) {
  ...
  if (e.shiftKey && component.lastSelectedIndex !== null) {
    ...
    // Case 4: plain click → clear all, then select this one
    component.shadowRoot.querySelectorAll('.draggable.selected')
      .forEach(el => el.classList.remove('selected'));
  ...
  component._isDragging = false; // reset lasso state after click
};
...
export function syncContainers(component) {
  const grab = container =>
    [...container.querySelectorAll(".draggable")].map(el => {
      const id = el.getAttribute("data-id");
      const original = component.fullData.find(obj => obj._id === id);
      return original || { _id: id, text: el.textContent.trim() };
    });
  ...
  component.columns.forEach(config => {
    ...
    if (container) {
      component.columnData[config.id] = grab(container);
    }
  });
 ...
 */
export function extractOrdersByIdKey(component, idKey = "_id") {
